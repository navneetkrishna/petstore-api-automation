...
pytestmark = [pytest.mark.regression, pytest.mark.store, pytest.mark.store]
...
class TestStoreInventory:
    ...
    @pytest.mark.smoke
    def test_str_i001_get_inventory_returns_200(self, store_service):
    ...
    @pytest.mark.schema
    def test_str_i002_inventory_schema_is_valid(self, store_service):
    ...
    def test_str_i003_inventory_changes_after_adding_sold_pet(
        self, store_service, pet_service):
        """Adding a pet with status='sold' increments the 'sold' count in inventory."""
        # Baseline
        before = store_service.json(store_service.get_inventory())
        ...
        # After
        after = store_service.json(store_service.get_inventory())
