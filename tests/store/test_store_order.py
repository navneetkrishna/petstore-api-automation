...
from util.assertions import ORDER_SCHEMA
from util.data_factory import make_order
...
class TestStoreOrder:
    ...
    @pytest.mark.smoke
    def test_str_o001_place_order_success(self, store_service, created_pet):
    ...
    def test_str_o003_place_order_empty_body(self, store_service):
        """POST /store/order with empty body — expect 4xx.
        Note: Petstore does not through 4xx error for empty payload
        """
        response = store_service.place_order({})
    ...
    @pytest.mark.negative
    def test_str_o004_place_order_zero_quantity(self, store_service, created_pet):
        ...
        if response.status_code == 200:
            store_service.delete_order(store_service.json(response)["id"])
    ...
    @pytest.mark.smoke
    def test_str_o005_get_order_by_id(self, store_service, created_order):
        """GET /store/order/{orderId} — Retrieve the order just placed."""
        response = store_service.get_order_by_id(created_order["id"])
    ...
    @pytest.mark.negative
    def test_str_o006_get_order_invalid_id(self, store_service):
        """GET /store/order/{orderId} with ID > 10 — expect 404 or server exception."""
        response = store_service.get_order_by_id(99999)

        assert response.status_code in (404, 500), \
            f"Expected 404 or 500 for order ID > 10, got {response.status_code}."

    @pytest.mark.negative
    def test_str_o007_get_deleted_order(self, store_service, created_pet):
        """GET an order after it has been deleted — expect 404."""
        payload = make_order(pet_id=created_pet["id"])
        create_response = store_service.place_order(payload)
        assert_status(create_response, 200)
        order_id = store_service.json(create_response)["id"]

        store_service.delete_order(order_id)

        get_response = store_service.get_order_by_id(order_id)
    ...
    def test_str_o008_delete_order(self, store_service, created_pet):
    ...
    @pytest.mark.negative
    def test_str_o009_delete_nonexistent_order(self, store_service):
        """DELETE a non-existent order — expect 404."""
        response = store_service.delete_order(999999)

        assert response.status_code in (404, 200), \
            "Documenting actual Petstore behaviour for non-existent order delete."

    @pytest.mark.schema
    def test_str_o010_order_response_schema(self, store_service, created_order):
        """Validate the GET /store/order response matches the Order schema."""
        response = store_service.get_order_by_id(created_order["id"])

        assert_status(response, 200)
        assert_schema(store_service.json(response), ORDER_SCHEMA)
