...
from util.assertions import assert_status, assert_schema, assert_field_equals
from util.assertions import PET_SCHEMA



class TestPetRead:
    """GET pet/{petid} and GET pet/{findByStatus}"""

    @pytest.mark.smoke
    def test_pet_r001_get_by_valid_id(self, pet_service, created_pet):
        """GET a pet by a valid ID that was just created — expect 200 and correct data."""
        response = pet_service.get_pet_by_id(created_pet["id"])

        # test assertion

        # assert response.status_code == 200
        # assert response.json().get("id") == created_pet["id"]
        # assert response.json().get("name") == created_pet["name"]

        """or use assertion helper method for validation"""
    ...
    def test_pet_r002_get_by_nonexistent_id(self, pet_service):
        """GET /pet/{petId} with an ID that does not exist — expect 404."""

        response = pet_service.get_pet_by_id(999_999_999)
        assert_status(response, 404)

    @pytest.mark.negative
    def test_pet_r003_get_by_id_zero(self, pet_service):
        """GET /pet/0 — expect 404 as ID 0 is not a valid pet."""

        response = pet_service.get_pet_by_id(0)
        assert_status(response, 404)


    @pytest.mark.negative
    def test_pet_r004_get_by_negative_id(self, pet_service):
        """GET /pet/-1 — expect 4xx for a negative ID."""
        response = pet_service.get_pet_by_id(-1)

        assert_status(response, 404)

    @pytest.mark.schema
    def test_pet_r005_response_schema_is_valid(self, pet_service, created_pet):
    ...
    def test_pet_r006_find_by_status(self, pet_service, status):
    ...
    @pytest.mark.smoke
    def test_pet_r007_find_by_status_items_match_status(self, pet_service):
        ...
        assert_status(response, 200)
        pets = pet_service.json(response)
    ...
    @pytest.mark.negative
    def test_pet_r008_find_by_invalid_status(self, pet_service):
        """GET /pet/findByStatus with an invalid status value — expect 400."""
        response = pet_service.find_by_status("invalid_status")
