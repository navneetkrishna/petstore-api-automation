...
from util.data_factory import make_pet, make_order
...
class TestOrderLifecycle:
    ...
    @pytest.mark.smoke
    def test_e2e_order_001_full_lifecycle(self, pet_service, store_service):
        ...
        pet_response = pet_service.create_pet(pet_payload)
        assert_status(pet_response, 200)
        pet_id = pet_service.json(pet_response)["id"]

        # Step 2: Place an order for that pet
        order_payload = make_order(pet_id=pet_id, status="placed")
        order_response = store_service.place_order(order_payload)
        assert_status(order_response, 200)
        order = store_service.json(order_response)
        order_id = order["id"]
        ...
        assert_field_equals(store_service.json(get_response), "id", order_id)

        # Step 4: Cancel the order
        delete_response = store_service.delete_order(order_id)
        assert_status(delete_response, 200)

        # Step 5: Verify the order is gone
        gone_response = store_service.get_order_by_id(order_id)
    ...
    def test_e2e_order_002_inventory_reflects_order_status(
        self, pet_service, store_service
    ):
        """Placing an order for a 'sold' pet is reflected in the store inventory count."""
        # Baseline inventory
        inv_before = store_service.json(store_service.get_inventory())
        ...
        # Inventory after
        inv_after = store_service.json(store_service.get_inventory())
