...
class TestPetLifecycle:
    ...
    @pytest.mark.smoke
    def test_e2e_pet_001_full_lifecycle(self, pet_service):
        ...
        pet = pet_service.json(create_response)
        pet_id = pet["id"]

        # Step 2: Read — verify it exists
        get_response = pet_service.get_pet_by_id(pet_id)
        assert_status(get_response, 200)
        assert_field_equals(pet_service.json(get_response), "id", pet_id)

        # Step 3: Update name
        updated = {**pet, "name": "LifecycleUpdatedName"}
        update_name_response = pet_service.update_pet(updated)
        assert_status(update_name_response, 200)
        assert_field_equals(pet_service.json(update_name_response), "name", "LifecycleUpdatedName")

        # Step 4: Update status to sold
        updated["status"] = "sold"
        update_status_response = pet_service.update_pet(updated)
        assert_status(update_status_response, 200)
        assert_field_equals(pet_service.json(update_status_response), "status", "sold")

        # Step 5: Verify status change persisted
        verify_response = pet_service.get_pet_by_id(pet_id)
        assert_status(verify_response, 200)
        assert_field_equals(pet_service.json(verify_response), "status", "sold")
        ...
        # Step 7: Verify gone
        gone_response = pet_service.get_pet_by_id(pet_id)
        assert gone_response.status_code == 404, \
            f"Pet {pet_id} should be gone after deletion, got {gone_response.status_code}."

    def test_e2e_pet_002_status_appears_in_find_by_status(self, pet_service):
        ...
        try:
            find_response = pet_service.find_by_status("pending")
            assert_status(find_response, 200)
            pets = pet_service.json(find_response)
