...
| Concept | API Framework  |
|---|---------------------------------------------------------|
| Base abstraction | `BaseService`                            |
| Domain objects | API Clients                                |
| Test data | `data_factory.py` (Faker)                       |
| State management | `created_pet`, `created_order`, `created_user` fixtures |
...
| Pytest 9.0.2         | Test runner and fixture framework |
| Faker 40.11.1        | Random test data generation       |
| jsonschema 4.26.0    | Response body schema validation   |
| allure-pytest 2.15.3 | Report management                 |
...
├── services/
│   ├── api/
│   │   ├── base_service.py               # Shared Session, headers, HTTP verbs
│   │   ├── pet_service.py                # /pet endpoint methods
│   │   ├── store_service.py              # /store endpoint methods
│   │   └── user_service.py               # /user endpoint methods
│   └── utils/
...
│   │   │   ├── test_pet_update.py       # PUT /pet (PET-U001–004)
...
│
├── allure-report/                       # Allure-created HTML reports
...
Each client inherits from `BaseService` and wraps a specific domain's endpoints.

| Client         | Base Path | Methods |
|----------------|---|---|
| `PetService`   | `/pet` | `create_pet`, `get_pet_by_id`, `find_by_status`, `find_by_tags`, `update_pet`, `update_pet_form`, `delete_pet` |
| `StoreService` | `/store` | `get_inventory`, `place_order`, `get_order_by_id`, `delete_order` |
| `UserService`  | `/user` | `create_user`, `create_users_with_list`, `login`, `logout`, `get_user`, `update_user`, `delete_user` |
...
git clone https://github.com/navneetkrishna/petstore-api-automation/tree/release/petstore-api-automation-v1.git
...
`src/api/base_service.py` is the foundation every API client inherits from — mirroring what `BasePage` does in the UI framework.
...
```
pet_service, store_service, user_service  (session scope — one instance per run)
